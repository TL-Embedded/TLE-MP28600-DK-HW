...
# Figure out the environment and paths.
# This may need editing if your installation is non standard.

IS_WINDOWS = sys.platform.startswith('win')

if IS_WINDOWS:
    # Windows
    KICAD_ROOT = os.path.expandvars("%PROGRAMFILES%/KiCad/7.0/bin")
    KICAD_PLUGIN_ROOT = os.path.expandvars("%USERPROFILE%/Documents/KiCad/7.0/3rdparty/plugins")

    KICAD_CLI = os.path.join(KICAD_ROOT, "kicad-cli.exe")
    KICAD_PYTHON = os.path.join(KICAD_ROOT, "python.exe")
else:
    # Linux (probably)
    KICAD_ROOT = "~/.local/share/kicad/7.0" 
    KICAD_PLUGIN_ROOT = os.path.join(KICAD_ROOT, "3rdparty/plugins")
    
    # These should be on the path.
    KICAD_CLI = "kicad-cli"
    KICAD_PYTHON = "python"

IBOM_SCRIPT = os.path.join(KICAD_PLUGIN_ROOT, "org_openscopeproject_InteractiveHtmlBom/generate_interactive_bom.py")
...
def export_pcb_image(input_pcb: str, output_file: str):
    ...
    if IS_WINDOWS:
        # This brings up mspaint, for you to paste the screenshot into.
        ...
        print("Please save the render of the PCB to \"{}\"".format(output_file))
